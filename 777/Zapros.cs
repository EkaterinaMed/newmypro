    ...
    public partial class Zapros : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (K==1){
                ...
                "')end";
                StreamWriter SW = new StreamWriter(new FileStream(@"C:\Users\dns\Desktop\777\hhparser\FileName.txt", FileMode.Create, FileAccess.Write));
                SW.Write(s[0]);
                SW.Close();
            }
            else if (K == 2){
            ...
            else
            {
                try
                {
                    ...
                    textBox2.DataBindings.Add("Text", bind, "name_vacancy");
                    richTextBox1.Clear();
                    
                    if (dataGridView1.Rows.Count == 0)
                    {
                        MessageBox.Show("По Вашему запросу ничего не найдено! Попробуйте еще раз!");
                    }
                }
                catch (Exception)
                {
                    //MessageBox.Show("В вашем запросе есть ошибка, попробуйте еще раз!Возможно был введен большой запрос");// обработка
        ...
        {
            SqlConnection cnn = new SqlConnection(@"Data Source=ASUS\SQLEXPRESS;Initial Catalog=hh;Integrated Security=True");
            SqlCommand cmd = new SqlCommand("infa1", cnn);
            ...
            da = new SqlDataAdapter(s1, cnn);
            string filter = string.Format("text_requerment>='{0}'", textBox2.Text);
            DataRow[] row = d2.Tables[0].Select(filter, "text_requerment ");
            if (row.Length == 0)
            {
                //MessageBox.Show("Требования не были найдены.");
                richTextBox1.Text = "";
                k++;
            }
            else
                ...
                if (row1.Length == 0)
                {
                    //MessageBox.Show("Обязанности не были найдены.");
                    richTextBox1.Text = "";
                    k++;
                }
                else
                ...
                if (row2.Length == 0)
                {
                    //MessageBox.Show("Условия не были найдены.");
                    richTextBox1.Text = "";
                    k++;
                }
                else
